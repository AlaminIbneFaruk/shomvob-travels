...
const Navbar = () => {
  ...
  // If the path is a dashboard path, do not render the navbar
  if (isDashboardPath) return null;

  return (
    <nav
      className={`fixed top-0 left-0 w-full bg-sky-400 shadow-md p-4 flex justify-between items-center z-50`}
    >
      <div className="flex items-center gap-2">
        <Link
          to="/"
          className="btn btn-ghost py-2 font-extrabold text-lg lg:text-3xl"
        >
          Shomvob Travels
        </Link>
      </div>
      ...
      <div className="hidden md:flex items-center gap-6">
        ...
        <Link
          to="/community"
          className={`${isActive("/community")} btn btn-outline py-2`}
        >
          Community
        </Link>
        <Link
          to="/about"
          className={`${isActive("/about")} btn btn-outline py-2`}
        >
          About Us
        </Link>
        <Link
          to="/trips"
          className={`${isActive("/trips")} btn btn-outline py-2`}
        >
          Trips
        </Link>
        ...
        {!user ? (
          <>
            <Link
              to="/login"
              className="btn-outline btn hover:underline bg-gradient-to-r from-cyan-400 to-blue-500"
            >
              Login
            </Link>
            <Link
      ...
      {menuOpen && (
        <div className="absolute top-16 left-0 w-full bg-sky-400 shadow-md flex flex-col items-center p-4 md:hidden">
          ...
          <Link
            to="/community"
            className={`${isActive("/community")} py-2`}
            onClick={() => setMenuOpen(false)}
          >
            Community
          </Link>
