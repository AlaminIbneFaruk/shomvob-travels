...
const fetchPackageDetails = async ({ queryKey }) => {
  const [, id] = queryKey;
  const response = await axios.get('/packageDetails.json'); // Note: This is static, see notes below
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-blue-200 p-20">
        <p className="text-xl text-red-600">
          {error ? `Error: ${error.message}` : 'Failed to load package details.'}
        </p>
      </div>
    );
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-blue-200 p-20 hover:bg-blue-400 transition duration-300">
      <div className="card w-full max-w-6xl shadow-xl bg-base-100 hover:bg-blue-300 transition duration-300 p-6 rounded-xl">
        <div className="card-body text-sky-900 transition duration-300">
          ...
          <button className="mt-6 w-full bg-blue-600 text-white font-semibold py-3 rounded-lg transition duration-300 hover:bg-white hover:text-blue-600 hover:border-blue-600 border border-transparent">
            🚀 Book Now
          </button>
        </div>
      </div>
